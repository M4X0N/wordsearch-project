...
def get_sentence_trees(word_lengths, text, indices):
    ...
    def get_sentences_recursion(char_index):
        ...
        try:
            ...
            for word_length in word_lengths[char_index]:
                curr_node = sentence_tree(
                    text[char_index:char_index + word_length])
                curr_node.children = get_sentences_recursion(
    ...
    for starting_idx in indices:
        if len(word_lengths[starting_idx]) > 0:
            ...
            current_sentence_tree.children = get_sentences_recursion(
                starting_idx)
...
def run_get_sentence_trees_MP(word_lengths, text):
    ...
    # get amount of found word indices to be allocated to each process
    skip = int(len(word_lengths) / os.cpu_count())
    # split word_lengths into N equal parts
    indices_list = [range(i, i + skip)
                    for i in range(0, len(word_lengths), skip)]
    all_sentence_trees = []
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor:
        results = executor.map(get_sentence_trees, repeat(
            word_lengths), repeat(text), indices_list)
